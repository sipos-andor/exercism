...
public static class CryptoSquare
{
    public static string Ciphertext(string plaintext) =>
        string.IsNullOrEmpty(plaintext) ? string.Empty : string.Join(" ", plaintext.Normalized().Rows().Cols());

    private static string Normalized(this string plaintext) =>
        new([.. plaintext.ToLower().Where(char.IsLetterOrDigit)]);
    ...
    private static IEnumerable<string> Cols(this string[] rows) =>
        Enumerable.Range(0, rows[0].Length).Select(i => new string(rows.Select(row => row[i]).ToArray()));
